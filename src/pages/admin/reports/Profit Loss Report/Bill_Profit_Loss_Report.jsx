...
export default function BillProfitLossReport() {
    // State management
    const [userData, setUserData] = useState(null)
    const [reportData, setReportData] = useState([])
    const [parties, setParties] = useState([]) // Added parties state for dropdown
    ...
    const fetchReportData = useCallback(async () => {
        ...
        try {
            ...
            const response = await api.get(`/company/${userData.selectedCompany.id}/bill-wise/profit-loss/report`, {
                params,
                headers: {
                    Authorization: `Bearer ${userData.accessToken}`,
                },
    ...
    const totals = React.useMemo(() => {
        return reportData.reduce(
            (acc, item) => ({
                totalSales: acc.totalSales + (item.totalSaleAmount || 0),
                totalProfit: acc.totalProfit + (item.profitAndLoss || 0),
            }),
    ...
    return (
        <div className={styles.reportContainer}>
            ...
            <div className={styles.reportHeader}>
                <h1 className={styles.reportTitle}>Bill Wise Profit & Loss Report</h1>
            ...
            <div className={styles.filtersSection}>
                <div className={styles.filtersGrid}>
                    ...
                    <div className={styles.filterGroup}>
                        <label htmlFor="partyId" className={styles.filterLabel}>
                            Party (Optional)
                        </label>
                        <select
                            id="partyId"
            ...
            {!error && reportData.length > 0 && (
                <div className={styles.summaryCards}>
                    <div className={styles.summaryCard}>
                        ...
                        <div className={styles.summaryValue}>{formatCurrency(totals.totalSales)}</div>
                    </div>
                    <div className={styles.summaryCard}>
                        <div className={styles.summaryLabel}>Total Profit/Loss</div>
                        <div className={`${styles.summaryValue} ${totals.totalProfit >= 0 ? styles.profit : styles.loss}`}>
                        ...
                        <table className={styles.reportTable}>
                            <thead className={styles.tableHeader}>
                                <tr>
                                    ...
                                    <th onClick={() => handleSort("invoiceNo")} className={styles.sortableHeader}>
                                        Invoice No
                                        {sortConfig.key === "invoiceNo" && (
                                            <span className={styles.sortIcon}>{sortConfig.direction === "asc" ? "↑" : "↓"}</span>
                                        )}
                                    </th>
                                    <th onClick={() => handleSort("partyName")} className={styles.sortableHeader}>
                                        Party Name
                                        {sortConfig.key === "partyName" && (
                                            <span className={styles.sortIcon}>{sortConfig.direction === "asc" ? "↑" : "↓"}</span>
                                        )}
                                    </th>
                                    <th onClick={() => handleSort("totalSaleAmount")} className={styles.sortableHeader}>
                                        Sale Amount
                                        {sortConfig.key === "totalSaleAmount" && (
                                            <span className={styles.sortIcon}>{sortConfig.direction === "asc" ? "↑" : "↓"}</span>
                                        )}
                                    </th>
                                    <th onClick={() => handleSort("profitAndLoss")} className={styles.sortableHeader}>
                                        Profit/Loss
                                        {sortConfig.key === "profitAndLoss" && (
                                            <span className={styles.sortIcon}>{sortConfig.direction === "asc" ? "↑" : "↓"}</span>
                                        )}
                                    </th>
                                </tr>
                            ...
                            <tbody className={styles.tableBody}>
                                {currentItems.map((item, index) => (
                                    <tr key={item.saleId || index} className={styles.tableRow}>
                                        ...
                                        <td className={styles.tableCell}>{item.invoiceNo || "-"}</td>
                                        <td className={styles.tableCell}>{item.partyName || "-"}</td>
                                        <td className={styles.tableCell}>{formatCurrency(item.totalSaleAmount)}</td>
                                        <td
                                            className={`${styles.tableCell} ${item.profitAndLoss >= 0 ? styles.profitCell : styles.lossCell}`}
                                        >
                                            {formatCurrency(item.profitAndLoss)}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {!loading && !error && reportData.length === 0 && (
                <div className={styles.emptyState}>
                    ...
                    <p className={styles.emptyDescription}>No profit and loss records found for the selected period.</p>
                </div>
            )}
        </div>
    )
