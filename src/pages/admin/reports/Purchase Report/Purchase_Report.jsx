...
import styles from "../../Styles/Report.module.css" // Reusing the same styles as the profit/loss report for consistency
...
export default function Purchase_Report() {
    ...
    const [filters, setFilters] = useState({
        period: "Today",
        partyId: "",
        purchaseTxnType: "", // Optional: PURCHASE or DEBIT_NOTE
        startDate: "",
        endDate: "",
    })
    ...
    const fetchReportData = useCallback(async () => {
        ...
        try {
            const params = {
                ...
                ...(filters.purchaseTxnType && { purchaseTxnType: filters.purchaseTxnType }),
                ...(filters.startDate && { startDate: filters.startDate }),
                ...(filters.endDate && { endDate: filters.endDate }),
            }

            const response = await api.get(`/company/${userData.selectedCompany.id}/purchase/report`, {
                params,
                headers: { Authorization: `Bearer ${userData.accessToken}` },
            })
    ...
    const totals = React.useMemo(() => {
        return reportData.reduce(
            (acc, item) => ({
                ...
                sendAmount: acc.sendAmount + (item.sendAmount || 0),
                balance: acc.balance + (item.balance || 0),
            }),
            { totalAmount: 0, sendAmount: 0, balance: 0 }
        )
    }, [reportData])
    ...
    return (
        <div className={styles.reportContainer}>
            ...
            <div className={styles.reportHeader}>
                <h1 className={styles.reportTitle}>Purchase Report</h1>
            ...
            <div className={styles.filtersSection}>
                <div className={styles.filtersGrid}>
                    ...
                    <div className={styles.filterGroup}>
                        <label htmlFor="purchaseTxnType" className={styles.filterLabel}>Transaction Type (Optional)</label>
                        <select
                            id="purchaseTxnType"
                            name="purchaseTxnType"
                            value={filters.purchaseTxnType}
                            onChange={handleFilterChange}
                            className={styles.filterSelect}
                            disabled={loading}
                        >
            ...
            {!error && reportData.length > 0 && (
                <div className={styles.summaryCards}>
                    ...
                    <div className={styles.summaryCard}>
                        ...
                        <div className={styles.summaryValue}>{formatCurrency(totals.sendAmount)}</div>
                    </div>
                    <div className={styles.summaryCard}>
                    ...
                    <div className={styles.tableContainer}>
                        <table className={styles.reportTable}>
                            <thead className={styles.tableHeader}>
                                <tr>
                                    <th onClick={() => handleSort("billDate")} className={styles.sortableHeader}>
                                        Bill Date
                                        {sortConfig.key === "billDate" && <span className={styles.sortIcon}>{sortConfig.direction === "asc" ? "↑" : "↓"}</span>}
                                    </th>
                                    <th onClick={() => handleSort("billNumber")} className={styles.sortableHeader}>
                                        Bill Number
                                    </th>
                                    <th onClick={() => handleSort("name")} className={styles.sortableHeader}>
                                        Party Name
                                    </th>
                                    <th onClick={() => handleSort("purchaseTransactionType")} className={styles.sortableHeader}>
                                        Type
                                    </th>
                                    ...
                                    <th onClick={() => handleSort("sendAmount")} className={styles.sortableHeader}>
                                        Paid Amount
                                    </th>
                            ...
                            <tbody className={styles.tableBody}>
                                {currentItems.map((item, index) => (
                                    <tr key={item.purchaseId || index} className={styles.tableRow}>
                                        <td className={styles.tableCell}>{formatDate(item.billDate)}</td>
                                        <td className={styles.tableCell}>{item.billNumber || "-"}</td>
                                        <td className={styles.tableCell}>{item.name || "-"}</td>
                                        <td className={styles.tableCell}>{item.purchaseTransactionType || "-"}</td>
                                        <td className={styles.tableCell}>{item.paymentType || "-"}</td>
                                        <td className={styles.tableCell}>{formatCurrency(item.totalAmount)}</td>
                                        <td className={styles.tableCell}>{formatCurrency(item.sendAmount)}</td>
            ...
            {!loading && !error && reportData.length === 0 && (
                <div className={styles.emptyState}>
                    ...
                    <p className={styles.emptyDescription}>No purchase records found for the selected filters.</p>
                </div>
            )}
        </div>
    )
