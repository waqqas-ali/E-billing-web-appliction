...
const CreateSaleOrder = () => {
  ...
  const units = [
    "CARTONS", "KILOGRAMS", "QUINTAL", "BOTTLES", "PIECES", "ROLLS", "NUMBERS", "PAIRS",
    "TABLETS", "MILLITRE", "BUNDLES", "BOX", "SQUARE_METERS", "BAGS", "CANS", "SQUARE_FEET",
  ]
  const taxTypes = ["WITHTAX", "WITHOUTTAX"]
  const taxRates = [
    "NONE", "EXEMPTED", "GST0", "IGST0", "GST0POINT25", "IGST0POINT25", "GST3", "IGST3",
    "GST5", "IGST5", "GST12", "IGST12", "GST18", "IGST18", "GST28", "IGST28",
  ]
  ...
  const ORDER_KEY_PREFIX = `saleOrderCounter_${companyId}_`

  const getNextOrderNumber = () => {
    if (!companyId) return "SO/NEW"
    const year = new Date().getFullYear()
    const key = `${ORDER_KEY_PREFIX}${year}`
    const counter = parseInt(localStorage.getItem(key) || "0", 10)
    return `SO/${year}/${counter + 1}`
  }

  const reserveNextOrderNumber = () => {
    if (!companyId) return "SO/NEW"
    const year = new Date().getFullYear()
    const key = `${ORDER_KEY_PREFIX}${year}`
  ...
  // Auto-generate preview order number for new orders
  useEffect(() => {
    if (editId || !companyId) return
    const previewNumber = getNextOrderNumber()
    setForm(prev => ({ ...prev, orderNo: previewNumber }))
  }, [editId, companyId])
  ...
  const handleOrderNoChange = (e) => {
    ...
    if (value === "SO" || value === "SO/" || value.startsWith("SO/")) {
      const nextOrder = reserveNextOrderNumber()
      setForm(prev => ({ ...prev, orderNo: nextOrder }))
      return
    }
            ...
            <div className={styles.formGrid}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  required
                  className={styles.input}
                  placeholder="Type SO to auto-generate"
                  disabled={!token || !companyId || loadingData || isEditMode}
                />
              </div>
