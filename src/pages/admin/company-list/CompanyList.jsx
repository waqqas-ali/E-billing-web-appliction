...
import api from "../../../utils/axiosInstance"; // 👈 Use shared API instance
...
function CompanyList() {
  ...
  // Keep userData in sync if changed elsewhere (e.g., logout, token refresh)
  useEffect(() => {
  ...
  const fetchCompanies = async () => {
    ...
    setError(null);
    try {
      const res = await api.get(`/owner/${userId}/companies`);
      setCompanies(res.data || []);
    } catch (err) {
      const message =
      ...
      setError(message);
      toast.error(message);
      // 401 will be handled by interceptor → may redirect to login
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpen = (company) => {
    ...
    toast.success(`Switched to ${company.bussinessName}`);
    navigate("/");
  };
