...
// import axios from "axios"
// import config from "../../../config/apiconfig"
// import styles from "../Styles/Form.module.css" // Same as CreateSale / CreateSaleOrder / CreateReturns
// import { toast } from "react-toastify"
// import {
//   Plus,
//   Trash2,
...
//           {/* ==== ITEMS TABLE (same as other forms) ==== */}
//           <div className={styles.formSection}>
//             <div className={styles.itemsHeader}>
//               <h2 className={styles.sectionTitle}>
//                 <Package size={20} />
...
//                         <input
//                           type="text"
//                           value={item.pricePerUnitTaxType === "WITHTAX" ? "Inc. Tax" : "Ex. Tax"}
//                           readOnly
//                           className={styles.tableInputReadonly}
//                         />
//                       </td>
...
//     setLoadingData(true);
//     try {
//       const res = await api.get(`/purchase/${purchaseId}`);
//       const p = res.data;
...
//       if (editId) {
//         await api.put(`/purchase/${editId}`, payload);
//         toast.success("Purchase updated!");
//       } else {
//         await api.post(`/company/${companyId}/create-purchase`, payload);
//         toast.success("Purchase created!");
//       }
...
const CreatePurchase = () => {
  ...
  const getNextBillNumberPreview = () => {
    if (!companyId) return "PB/NEW";
    const year = new Date().getFullYear();
    const key = `purchaseBillCounter_${companyId}_${year}`;
    const counter = parseInt(localStorage.getItem(key) || "0", 10);
    return `PB/${year}/${counter + 1}`;
  };

  const reserveNextBillNumber = () => {
    if (!companyId) return "PB/NEW";
    const year = new Date().getFullYear();
    const key = `purchaseBillCounter_${companyId}_${year}`;
    ...
    return `PB/${year}/${counter}`;
  };

  // Show preview bill number when creating new purchase
  useEffect(() => {
    if (editId || !companyId) return;
    const preview = getNextBillNumberPreview();
  ...
  const handleBillNumberChange = (e) => {
    ...
    if (value === "PB" || value === "PB/" || value.startsWith("PB/")) {
      const nextNumber = reserveNextBillNumber();
      setForm(prev => ({ ...prev, billNumber: nextNumber }));
      return;
    }

    setForm(prev => ({ ...prev, billNumber: value }));
  };
            ...
            <div className={styles.formGrid}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  required
                  className={styles.input}
                  placeholder="Type PB to auto-generate"
