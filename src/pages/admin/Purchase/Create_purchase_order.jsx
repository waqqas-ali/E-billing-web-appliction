...
//           {/* ==== ITEMS TABLE (consistent with other forms) ==== */}
//           <div className={styles.formSection}>
//             <div className={styles.itemsHeader}>
//               <h2 className={styles.sectionTitle}>
//                 <Package size={20} />
...
//                           min="0.01"
//                           value={item.perUnitRate}
//                           onChange={(e) => handleItemChange(idx, "perUnitRate", e.target.value)}
//                           required
//                           className={styles.tableInput}
//                           disabled={!token || !companyId || loadingData}
//                         />
...
//       if (editId) {
//         await api.put(`/purchase-order/${editId}`, payload);
//         toast.success("Purchase Order updated!");
//       } else {
//         await api.post(`/company/${companyId}/created/purchase-order`, payload);
//         toast.success("Purchase Order created!");
//       }
...
const CreatePurchaseOrder = () => {
  ...
  // ─── Order No Auto-generation Logic ───────────────────────────────
  const getNextOrderNumberPreview = () => {
    if (!companyId) return "PO/NEW";
    const year = new Date().getFullYear();
    const key = `purchaseOrderCounter_${companyId}_${year}`;
    const counter = parseInt(localStorage.getItem(key) || "0", 10);
    return `PO/${year}/${counter + 1}`;
  };
  ...
  // Show preview order number for new orders
  useEffect(() => {
    if (editId || !companyId) return;
    const preview = getNextOrderNumberPreview();
    setForm(prev => ({ ...prev, orderNo: preview }));
  }, [editId, companyId]);

  const handleOrderNoChange = (e) => {
    let value = e.target.value.trim().toUpperCase();

    if (value === "PO" || value === "PO/" || value.startsWith("PO/")) {
      const nextNumber = reserveNextOrderNumber();
      setForm(prev => ({ ...prev, orderNo: nextNumber }));
      return;
    }
            ...
            <div className={styles.formGrid}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  required
                  className={styles.input}
                  placeholder="Type PO to auto-generate"
