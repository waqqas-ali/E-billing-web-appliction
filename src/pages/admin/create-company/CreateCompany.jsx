...
function CreateCompany() {
  ...
  // Sync userData if changed elsewhere (e.g., logout or token refresh)
  useEffect(() => {
  ...
  const fetchCompany = async (id) => {
    ...
    setError(null);
    try {
      const response = await api.get(`/get/company/${id}`);
      const company = response.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (companyEditId) {
        await api.put(`/update/company/${companyEditId}`, formData);
        toast.success("Company updated successfully!");
      } else {
        await api.post(`/create/company/by/owner/${userId}`, formData);
      ...
      const message =
        err.response?.data?.message ||
        "Failed to save company. Please check required fields and try again.";
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
